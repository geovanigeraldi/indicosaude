	...
	foreach($chats as $chat)
	{
		if(++$count < DATA_ITEM_LOADS || $chat["closed"] == $last)
		{
			if($_external && $permission)
				$xml .= "<c cid=\"".base64_encode($chat["chat_id"])."\" iid=\"".base64_encode($chat["internal_id"])."\" gid=\"".base64_encode($chat["group_id"])."\" eid=\"".base64_encode($chat["external_id"])."\" en=\"".base64_encode($chat["fullname"])."\" ts=\"".base64_encode($chat["time"])."\" cl=\"".base64_encode($last = $chat["closed"])."\" te=\"".base64_encode($chat["endtime"])."\" em=\"".base64_encode($chat["email"])."\" ac=\"".base64_encode($chat["area_code"])."\" co=\"".base64_encode($chat["company"])."\" il=\"".base64_encode($chat["iso_language"])."\" ho=\"".base64_encode($chat["host"])."\" ip=\"".base64_encode($chat["ip"])."\" gzip=\"".base64_encode($chat["gzip"])."\">".base64_encode($chat["html"])."</c>\r\n";
			else
...
function buildExtern($objectCount=0)
{
	...
	if(count($VISITOR) > 0)
	{
		$builder = new ExternalXMLBuilder($CONFIG,$INTERNAL[CALLER_SYSTEM_ID],$VISITOR,(isset($_POST[POST_INTERN_RESYNC]) || LOGIN || $_POST[POST_INTERN_XMLCLIP_HASH_TRACKING] == XML_CLIP_NULL),($GROUPS[$INTERNAL[CALLER_SYSTEM_ID]->Groups[0]]->IsExternal));
